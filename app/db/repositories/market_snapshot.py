...
class MarketSnapshotRepository:
    ...
    async def persist(self, snapshot: MarketSnapshot) -> int:
        ...
        for key, listings in grouped.items():
            ...
            for item in listings:
                ...
                if listing is None:
                    listing = Listing(gift_id=gift.id, marketplace=item.marketplace, external_id=item.listing_id, price_ton=item.price_ton, seller=item.seller, url=str(item.url) if item.url else None, first_seen_at=item.observed_at, last_seen_at=item.observed_at, active=True)
        ...
        await self.session.execute(update(Listing).where(Listing.marketplace == snapshot.marketplace, Listing.last_seen_at < now).values(active=False))
    ...
    async def _get_or_create_gift(self, key: str, item) -> Gift:
        ...
        if gift is None:
            gift = Gift(canonical_id=key, gift_number=item.gift_number, name=item.name, model=item.model, image_url=None)
