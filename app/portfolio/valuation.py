...
async def resolve_gift(session: AsyncSession, *, nft_address: str, collection_address: str | None, metadata: dict) -> tuple[Gift | None, str, Decimal | None]:
    exact = await session.scalar(select(Gift).where(Gift.canonical_id == nft_address))
    if exact:
        return exact, "canonical_nft_address", Decimal("100")
    ...
    model = normalize(metadata_value(metadata, "model"))
    if not name or not model:
        return None, "unresolved", None
    stmt = select(Gift).where(Gift.collection_id == collection.id, Gift.name.is_not(None), Gift.model.is_not(None))
    ...
    if len(matches) != 1:
        return None, "ambiguous_identity", None
    return matches[0], "collection_name_model_exact", Decimal("85")
