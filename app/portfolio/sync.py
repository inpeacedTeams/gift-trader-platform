...
async def sync_portfolios() -> PortfolioSyncReport:
    ...
    async with SessionLocal() as session:
        ...
        synced_wallets = holdings_count = unavailable = alerts_count = 0
        user_ids = {wallet.user_id for wallet in wallets}
        for wallet in wallets:
            try:
                account = await client.account(wallet.address)
                ...
                for item in nft_items:
                    ...
                    if not address:
                        continue
                    address = str(address); seen.append(address)
                    ...
                    if holding is None:
                        holding = PortfolioHolding(wallet_id=wallet.id, nft_address=address); session.add(holding)
                    ...
                    holding.name = metadata.get("name") or holding.name; holding.image_url = metadata.get("image") or holding.image_url
                    holding.estimated_price_ton = price; holding.observed_at = datetime.now(timezone.utc); holdings_count += 1
                if seen:
        ...
        for user_id in user_ids:
            valuation = await _record_user_valuation(session, user_id, wallets, client)
            alerts_count += await _evaluate_alerts(session, user_id, valuation)
        await session.commit()
    return PortfolioSyncReport(len(wallets), synced_wallets, holdings_count, unavailable, alerts_count)

async def _record_user_valuation(session, user_id: int, wallets, client):
    user_wallets = [wallet for wallet in wallets if wallet.user_id == user_id]
    ...
    nft_value = sum((item.estimated_price_ton or Decimal("0") for item in holdings), Decimal("0"))
    ton_balance = Decimal("0")
    for wallet in user_wallets:
        try:
            ton_balance += client.ton_balance(await client.account(wallet.address))
        except SourceUnavailable:
            pass
    valuation = PortfolioValuation(user_id=user_id, total_ton=ton_balance + nft_value, ton_balance=ton_balance, nft_value_ton=nft_value, asset_count=len(holdings), observed_at=datetime.now(timezone.utc))
    session.add(valuation); await session.flush(); return valuation

async def _evaluate_alerts(session, user_id: int, current: PortfolioValuation) -> int:
    ...
    count = 0
    change = ((current.total_ton - previous.total_ton) / previous.total_ton * Decimal("100")) if previous.total_ton else Decimal("0")
    for rule in rules:
        ...
        if not triggered:
            continue
        session.add(AlertEvent(rule_id=rule.id, user_id=user_id, message=f"Portfolio {rule.rule_type.replace('_', ' ')}: {current.total_ton} TON ({change:+.2f}%)", observed_value=current.total_ton))
        count += 1
    return count
