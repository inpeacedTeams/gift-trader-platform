...
class AlertCreate(BaseModel):
    ...
    rule_type: str = Field(pattern="^(price_below|price_above|change_percent|listed_below)$")
...
async def watchlist(user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    rows = (await session.scalars(select(WatchlistItem).where(WatchlistItem.user_id == user.id).order_by(WatchlistItem.created_at.desc()))).all()
    return {"items": [{"id": row.id, "gift_id": row.gift_id, "created_at": row.created_at} for row in rows]}
...
async def remove_watchlist(gift_id: int, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    await session.execute(delete(WatchlistItem).where(WatchlistItem.user_id == user.id, WatchlistItem.gift_id == gift_id))
    await session.commit()

@router.get("/portfolio/wallets")
async def wallets(user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    rows = (await session.scalars(select(PortfolioWallet).where(PortfolioWallet.user_id == user.id).order_by(PortfolioWallet.created_at.desc()))).all()
    return {"items": [{"id": row.id, "address": row.address, "label": row.label, "created_at": row.created_at} for row in rows]}
...
async def add_wallet(body: WalletCreate, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    ...
    if wallet is None:
        wallet = PortfolioWallet(user_id=user.id, address=body.address, label=body.label)
        session.add(wallet)
        await session.commit()
...
async def remove_wallet(wallet_id: int, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    await session.execute(delete(PortfolioWallet).where(PortfolioWallet.id == wallet_id, PortfolioWallet.user_id == user.id))
    await session.commit()

@router.get("/alerts/rules")
async def alert_rules(user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    rows = (await session.scalars(select(AlertRule).where(AlertRule.user_id == user.id).order_by(AlertRule.created_at.desc()))).all()
    return {"items": [{"id": row.id, "gift_id": row.gift_id, "rule_type": row.rule_type, "threshold": row.threshold, "is_active": row.is_active, "created_at": row.created_at} for row in rows]}
...
async def create_alert(body: AlertCreate, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    if body.gift_id is not None and not await _gift_exists(session, body.gift_id):
        raise HTTPException(404, "Gift not found")
    rule = AlertRule(user_id=user.id, gift_id=body.gift_id, rule_type=body.rule_type, threshold=body.threshold)
    session.add(rule)
    await session.commit()
    await session.refresh(rule)
    return {"id": rule.id, "gift_id": rule.gift_id, "rule_type": rule.rule_type, "threshold": rule.threshold, "is_active": rule.is_active}
...
async def delete_alert(rule_id: int, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    await session.execute(delete(AlertRule).where(AlertRule.id == rule_id, AlertRule.user_id == user.id))
    await session.commit()
...
async def alert_events(unread_only: bool = False, limit: int = Query(default=50, ge=1, le=200), user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    ...
    return {"items": [{"id": row.id, "rule_id": row.rule_id, "gift_id": row.gift_id, "message": row.message, "observed_value": row.observed_value, "is_read": row.is_read, "created_at": row.created_at} for row in rows]}
