...
async def alert_rules(user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    rows = (await session.scalars(select(AlertRule).where(AlertRule.user_id == user.id).order_by(AlertRule.created_at.desc()))).all(); return {"items": [{"id": row.id, "gift_id": row.gift_id, "rule_type": row.rule_type, "threshold": row.threshold, "is_active": row.is_active, "created_at": row.created_at} for row in rows]}
...
async def create_alert(body: AlertCreate, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    ...
    rule = AlertRule(user_id=user.id, gift_id=body.gift_id, rule_type=body.rule_type, threshold=body.threshold); session.add(rule); await session.commit(); await session.refresh(rule); return {"id": rule.id, "gift_id": rule.gift_id, "rule_type": rule.rule_type, "threshold": rule.threshold, "is_active": rule.is_active}
...
async def alert_events(unread_only: bool = False, limit: int = Query(default=50, ge=1, le=200), user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    ...
    rows = (await session.scalars(stmt.order_by(AlertEvent.created_at.desc()).limit(limit))).all(); return {"items": [{"id": row.id, "rule_id": row.rule_id, "gift_id": row.gift_id, "message": row.message, "observed_value": row.observed_value, "is_read": row.is_read, "created_at": row.created_at} for row in rows]}
