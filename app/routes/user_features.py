...
async def add_watchlist(gift_id: int, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    ...
    if item is None:
        item = WatchlistItem(user_id=user.id, gift_id=gift_id); session.add(item); await session.commit(); await session.refresh(item)
...
async def add_wallet(body: WalletCreate, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    ...
    if wallet is None:
        wallet = PortfolioWallet(user_id=user.id, address=body.address, label=body.label); session.add(wallet); await session.commit(); await session.refresh(wallet)
...
async def create_alert(body: AlertCreate, user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    ...
    rule = AlertRule(user_id=user.id, gift_id=body.gift_id, rule_type=body.rule_type, threshold=body.threshold); session.add(rule); await session.commit(); await session.refresh(rule)
