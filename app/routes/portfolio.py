...
from app.db.models import Gift, PortfolioHolding, PortfolioWallet, PriceSnapshot, User
    ...
    for wallet in wallets:
        try:
            ...
            for item in nft_items:
                ...
                gift = await session.scalar(select(Gift).where(Gift.canonical_id == str(address)))
                price = None
                if gift:
                ...
                if existing is None:
                    existing = PortfolioHolding(wallet_id=wallet.id, nft_address=str(address), collection_address=str(item.get("collection", {}).get("address")) if isinstance(item.get("collection"), dict) else None, name=metadata.get("name"), image_url=metadata.get("image"), estimated_price_ton=price)
                    session.add(existing)
                else:
                    existing.name = metadata.get("name") or existing.name
                    existing.image_url = metadata.get("image") or existing.image_url
                    existing.estimated_price_ton = price
            ...
            unavailable.append({"wallet_id": wallet.id, "address": wallet.address, "error": exc.reason})
    await session.commit()
    return {"data_mode": "live-tonapi", "wallets": wallet_results, "total_assets": total_assets, "estimated_nft_value_ton": total_ton, "unavailable": unavailable}
