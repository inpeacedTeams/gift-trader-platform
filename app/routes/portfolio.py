...
from app.portfolio.resolver_telemetry import summary
...
async def portfolio_overview(user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    wallets = list((await session.scalars(select(PortfolioWallet).where(PortfolioWallet.user_id == user.id).order_by(PortfolioWallet.created_at.desc()))).all()); client = TonapiPortfolioClient(); wallet_results = []; total_ton = Decimal("0"); total_assets = valued_assets = 0; unavailable = []
    for wallet in wallets:
        try:
            ...
            for item in nft_items:
                ...
                gift, source, confidence, candidates = await resolve_gift(session, nft_address=address, collection_address=collection_address, metadata=metadata); price = await latest_floor(session, gift)
                ...
                holdings.append({"nft_address": address, "name": metadata.get("name"), "image_url": metadata.get("image"), "estimated_price_ton": price, "valuation_source": source, "valuation_confidence": confidence, "candidate_count": candidates}); total_assets += 1
...
@router.get("/resolver-summary")
async def resolver_summary(user: User = Depends(current_user), session: AsyncSession = Depends(get_session)):
    return {"data_mode": "telemetry", "outcomes": await summary(session)}
