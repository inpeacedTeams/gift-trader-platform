...
from app.routes.jobs import require_admin
...
@router.get("/snapshots", response_model=MarketsResponse, dependencies=[Depends(require_admin)])
async def snapshots(
    collection: list[str] = Query(default=[]),
    portals_endpoint: str | None = None,
    session: AsyncSession = Depends(get_session),
):
    """Force a crawl and return what it found.

    Admin only: this walks every marketplace end to end. The product reads
    stored data through /overview, /gifts and /events instead.
    """
    ...
    unavailable = [
        {"marketplace": item["marketplace"], "error": item["reason"], "listings": []}
        for item in result.unavailable
    ]
    return MarketsResponse(
        markets=[snapshot.model_dump() for snapshot in result.snapshots], unavailable=unavailable
    )


@router.get("/health", dependencies=[Depends(require_admin)])
async def source_health(collection: list[str] = Query(default=[]), portals_endpoint: str | None = None):
    """Live probe of every collector. Use /sources/status for the cheap read."""
    result = await collect(_parsers(collection, portals_endpoint))
