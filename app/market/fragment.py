...
class FragmentParser(MarketParser):
    ...
    async def snapshot(self) -> MarketSnapshot:
        ...
        html = await self.http.get_text("fragment", self.url, headers={"User-Agent": "GiftTrader/0.1"})
        ...
        for node in soup.select("[data-price], .tm-grid-item, .tm-gift-item"):
            ...
            if not raw_price:
                continue
            raw_price = raw_price if isinstance(raw_price, str) else raw_price.get("data-price")
            ...
            listings.append(Listing(
                marketplace="fragment", listing_id=absolute, gift_id=absolute,
                name=node.get("data-name") or node.get_text(" ", strip=True)[:120],
                price_ton=Decimal(match.group().replace(",", ".")),
                url=HttpUrl(absolute), observed_at=now, source_url=HttpUrl(self.url),
            ))
        return MarketSnapshot(marketplace="fragment", observed_at=now, listings=listings, source_url=HttpUrl(self.url))
