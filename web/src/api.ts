import type { ArbitrageResponse, MarketResponse, Snapshot } from "./types";
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (response.status === 401) { clearToken(); throw new Error("Authentication required"); }
...
export async function authenticateTelegram(): Promise<User | null> {
  const initData = telegramInitData();
  if (!initData) return null;
  const result = await request<{ access_token: string; user: User }>("/auth/telegram", { method: "POST", body: JSON.stringify({ init_data: initData }) });
